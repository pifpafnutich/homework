...
def is_prime(number):
    ...
    Более экономный вариант для списка: найти самое большое число, 
    вызвать рекурсивную функцию, которая найдёт все простые числа до него,
    затем проверить, входят ли другие элементы списка в простые числа
    """
    print(number)
    if number < 2:
        return False
